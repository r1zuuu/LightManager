...
from webapp.mqtt_client import mqtt_manager


async def create_switch(switch_in: schemas.SwitchCreate, db: Session) -> models.Switch:
...
def get_switches(db: Session) -> list[models.Switch]:
    return db.query(models.Switch).all()


async def update_switch_state(
    switch_id: str, state_in: schemas.SwitchUpdate, db: Session
) -> models.Switch:
    ...
    if db_switch.is_on and not state_in.is_on:
        ...
        if db_switch.last_turned_on:
            delta = (
                datetime.datetime.utcnow() - db_switch.last_turned_on
            ).total_seconds()
